...
REDUCTION_F = 1                     #only for large datasets
RANDOM_S = 0                        #Random Seed for everything except the DCA              #
DoE_PARAM = {"le_strategy":"leV1", "p_vec":None, "random_seed":0,"start":0,"stop":20,"step":5}
...
def run_single_pipeline(ds_name, cl_name, reduction_f,random_s, doe_params, exp_fold, save_files, vis_data=VIS_DATA):
    #CONSTANTS
    ...
    df_, trace_M_= apply_label_errors(train_test_df=current_ds, cl_dict=cl_dict, ds_=ds_name, doe_param= doe_params, exp_folder=EXP_FOLD)
    res_dict = {"df_":df_,
                "trace_M_":trace_M_,
...
if __name__ == "__main__":
    res_obj = run_single_pipeline(ds_name=DATASET_NAME, cl_name=CLASSIFIER_NAME, reduction_f=REDUCTION_F,random_s=RANDOM_S,
